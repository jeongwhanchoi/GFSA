...
class Graphormer(pl.LightningModule):
    def __init__(
        ...
        encoders = [EncoderLayer(hidden_dim, ffn_dim, dropout_rate, attention_dropout_rate, num_heads, mode, K)
    ...
    def add_model_specific_args(parent_parser):
        ...
        ######################################
        ############## GFSA ##############
        parser.add_argument("--mode", type=str, default="vanilla", help="vanilla or gfsa")
        parser.add_argument("--K",  type=int, default=0, help="order of polynomial")
        ######################################
        ######################################
...
class MultiHeadAttention(nn.Module):
    def __init__(self, hidden_size, attention_dropout_rate, num_heads, mode='', K=0):
        ...
        if mode == 'gfsa':
            self.K = K
            self.w_K = nn.Parameter(torch.zeros(self.num_heads))
    ...
    def forward(self, q, k, v, attn_bias=None):
        ...
        if self.mode == 'gfsa':
            ...
            high_order = x + (K-1) * (x2-x)

            w_K = self.w_K[None,:,None,None]
            x = x + w_K * high_order
...
class EncoderLayer(nn.Module):
    def __init__(self, hidden_size, ffn_size, dropout_rate, attention_dropout_rate, num_heads, mode, K):
        super(EncoderLayer, self).__init__()

        self.self_attention_norm = nn.LayerNorm(hidden_size)
        self.self_attention = MultiHeadAttention(
            hidden_size, attention_dropout_rate, num_heads, mode, K)
