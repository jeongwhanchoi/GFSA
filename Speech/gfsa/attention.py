...
import math
from speechbrain.nnet.mha_layer import MultiheadAttention as CustomMHALayer
...
class RelPosMHAXL2(nn.Module):
    ...
    >>> inputs = torch.rand([6, 60, 512])
    >>> pos_emb = torch.rand([1, 2*60-1, 512])
    >>> net = RelPosMHAXL2(num_heads=8, embed_dim=inputs.shape[-1])
    >>> outputs, attn = net(inputs, inputs, inputs, pos_emb)
    >>> outputs.shape
    torch.Size([6, 60, 512])
    """
    ...
    def __init__(
        ...
        vdim=None,
        mask_pos_future=False,
        mode=None, beta=0, h=0
    ):
        super(RelPosMHAXL2, self).__init__()
        self.embed_dim = embed_dim
        self.vdim = vdim if vdim is not None else embed_dim
        self._qkv_same_embed_dim = self.vdim == embed_dim
        self.mask_pos_future = mask_pos_future
    ...
    def forward(
        ...
        if self.mode == 'rd':
            pass
        elif self.mode == 'rd_gf':
            attn_score2 = torch.matmul(attn_score, attn_score)
            high_order = attn_score + (self.h-1) * (attn_score2-attn_score)
            attn_score = attn_score + self.beta[None,:,None,None] * high_order
