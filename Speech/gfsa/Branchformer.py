...
from speechbrain.nnet.attention import RelPosMHAXL, RelPosMHAXL2, MultiheadAttention, MultiheadAttention2
...
class BranchformerEncoderLayer(nn.Module):
    ...
    def __init__(
        ...
        if attention_type == "regularMHA":
            ...
            self.self_att = MultiheadAttention2(
                nhead=nhead,
                d_model=d_model,
                dropout=dropout,
                kdim=kdim,
...
class BranchformerEncoder(nn.Module):
    ...
    def forward(
        ...
        if self.attention_type == "RelPosMHAXL2":
            if pos_embs is None:
                raise ValueError(
                    "The chosen attention type for the Branchformer is RelPosMHAXL. For this attention type, the positional embeddings are mandatory"
                )
