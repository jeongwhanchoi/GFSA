...
class TransformerInterface(nn.Module):
    ...
    def __init__(
        ...
        assert attention_type in ["regularMHA", "regularMHAGF", "RelPosMHAXL", "RelPosMHAXL2"]
        ...
        elif attention_type == "RelPosMHAXL2":
            self.positional_encoding = RelPosEncXL(d_model)
            self.positional_encoding_decoder = PositionalEncoding(
                d_model, max_length
            )
        ...
        if num_encoder_layers > 0:
            ...
            if encoder_module == "transformergf":
                self.encoder = TransformerEncoder(
                    nhead=nhead,
                    num_layers=num_encoder_layers,
                    d_ffn=d_ffn,
...
class TransformerEncoderLayer(nn.Module):
    ...
    def __init__(
        ...
        if attention_type == "regularMHA":
        ...
        elif attention_type == "regularMHAGF":
            self.self_att = sb.nnet.attention.MultiheadAttention2(
                nhead=nhead,
                d_model=d_model,
                dropout=dropout,
                kdim=kdim,
            ...
            self.self_att = sb.nnet.attention.RelPosMHAXL2(
                d_model, nhead, dropout, mask_pos_future=causal
            )
...
class TransformerDecoderLayer(nn.Module):
    ...
    def __init__(
        ...
        if attention_type == "regularMHA":
            ...
            self.self_attn = sb.nnet.attention.MultiheadAttention2(
                nhead=nhead,
                d_model=d_model,
                kdim=kdim,
                vdim=vdim,
            ...
            self.mutihead_attn = sb.nnet.attention.MultiheadAttention2(
                nhead=nhead,
                d_model=d_model,
                kdim=kdim,
                vdim=vdim,
            ...
            self.self_attn = sb.nnet.attention.RelPosMHAXL2(
                d_model, nhead, dropout, mask_pos_future=causal
            )
            self.mutihead_attn = sb.nnet.attention.RelPosMHAXL2(
                d_model, nhead, dropout, mask_pos_future=causal
            )
